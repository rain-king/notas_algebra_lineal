...
% Ya que para $v-w\in U$, $v+U=w+U$, no tenemos una representación
% única de un elemento de $V/U$, para arreglar esto definimos las clases
% de equivalencia. Como acabamos de demostrar $v-w\in U \iff v+U=W+U$,
% así que definimos la relación de equivalencia $~$ por
% $u~v \iff u-v\in U$. Y definimos la clase de equivalencia por
% $[u]:=\{w:s~w\}$. Luego, identificamos $[u]$ con $u$.
% De manera que ahora $v+U$ está bien definido.
...
\begin{enumerate}
    ...
    \item Advanced Linear Algebra 3ra edición, por Roman, Steve.
\end{enumerate}
\end{document}
