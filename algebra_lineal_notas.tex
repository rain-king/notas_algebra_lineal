...
\subsubsection*{Ejemplo}
El conjunto $O$ que consiste de $0$ genera al espacio vectorial $V$
que consiste de $0$. Sin embargo $O$, como ya vimos, no es linealmente independiente, por lo tanto no es base para $V$.
(Excluimos casos triviales donde el campo permite que $\{0\}$ sea base. Si es que estos casos existen.)
...
\subsection{Rango}
Dado $T: V\rightarrow W$, el rango de $T$ es
$T(V) = \{Tx: x\in V\}$. Y lo denotamos por $\ran T$.

Se puede demostrar, que análogo a $\ker T$, $\ran T$ forma un
subespacio de $W$: Sea $y\in \spn\ran T$, entonces
$y=\sum_i \beta_i y_i$ donde $y_i=Tx_i$ para algún $x_i\in V$.
Luego $y=T(\sum_i \beta_i x_i) = Tx$ para
$x=\sum_i \beta_i x_i \in V$; $y\in\ran T$.

\subsubsection*{Sobreyectividad}
Trivialmente, $T: V\rightarrow W$ es sobreyectiva si $\ran T = W$.

\subsection{Importancia del kernel y el rango}
Si tenemos el sistema de ecuaciones lineales homogéneo, $Ax=0$,
es de interés saber si hay otras soluciones además de $x=0$.
Esto se puede responder si encontramos que la transformación $T_A$
correspondiente a $A$, tiene kernel distinto de $\{0\}$.

Si tenemos un sistema de ecuaciones lineales $Ax=b$, es de
interés si existen soluciones para $x$, y si existe alguna,
si es única. Esto se podrá responder con teoremas a partir de propiedades tanto del rango como del kernel de la transformación $T_A$.
...
$1$ hasta $m$ forma una base para $\ran T$. Primero, como $x$ era arbitrario, $\spn \{Tv_j\}_j = \ran T$.
